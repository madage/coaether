...
| 组件 | 职责 | 当前存在？ |
|------|------|-----------|
| **Message Bus** | 端点注册、消息路由、消息投递。运行在 Server 进程中 | 部分（WSHub 雏形） |
| **Agent Runtime** | 在每个节点上运行，托管 Agent Backend，对外暴露为消息端点 | 否（当前是 agent-node） |
| **WebUI** | 用户界面，消息流渲染，不再是终端 | 是（React 应用） |
| **Agent Backend** | 实际的 AI 工具接入层（API 调用 / CLI 封装） | 否（当前是 PTY 子进程） |
...
### 阶段 2：Agent Runtime + API Adapter（约 2 周）
